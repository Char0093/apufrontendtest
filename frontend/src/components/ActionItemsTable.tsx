...
export const ActionItemsTable: React.FC<ActionItemsTableProps> = ({
  ...
  return (
    <div className="space-y-4 font-sans relative">
      ...
      {toastMessage && (
        <div className="fixed bottom-6 right-6 z-50 flex items-center gap-2.5 bg-slate-900 text-white px-4 py-3 rounded-2xl shadow-2xl border border-indigo-500/40 text-xs font-semibold animate-in fade-in slide-in-from-bottom-4 duration-200">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2 text-xs">
          ...
          <div className="relative flex items-center">
            <UserCheck className="w-3.5 h-3.5 absolute left-2.5 text-indigo-600 dark:text-indigo-400 pointer-events-none" />
            <select
              value={activeUser}
              onChange={(e) => setActiveUser(e.target.value)}
              className="pl-8 pr-3 py-1.5 rounded-xl bg-indigo-50 dark:bg-indigo-950/60 border border-indigo-200 dark:border-indigo-800 text-indigo-900 dark:text-indigo-200 font-bold text-xs focus:outline-hidden cursor-pointer shadow-2xs"
            >
                    ...
                    >
                      ...
                      <td className="py-3.5 px-4 font-medium text-slate-900 dark:text-white max-w-xs sm:max-w-md">
                        <div className="flex items-start gap-2">
                          <span className="mt-0.5 text-indigo-600 dark:text-indigo-400 font-bold text-xs">•</span>
                      ...
                      <td className="py-3.5 px-4 whitespace-nowrap">
                        <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold border ${
                          ...
                          {assigneeName}
                          {isMyTask && (
                            <span className="text-[10px] bg-indigo-200 dark:bg-indigo-900 text-indigo-800 dark:text-indigo-200 px-1.5 py-0.2 rounded-md font-bold">
                              You
                            </span>
                          )}
                        </span>
                      ...
                      <td className="py-3.5 px-4 whitespace-nowrap">
                        <span className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-semibold border ${
                          ...
                          item.status === 'In Progress' ? 'bg-indigo-100 dark:bg-indigo-950/60 text-indigo-800 dark:text-indigo-300 border-indigo-200 dark:border-indigo-800' :
                          ...
                          {item.status === 'In Progress' && <Clock className="w-3 h-3 text-indigo-600 dark:text-indigo-400" />}
